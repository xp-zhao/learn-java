...
public class AopUtil {
  public static final String PROXY_PLUGIN_PREFIX = "pluginAdvisor" + StrUtil.UNDERLINE;

  private AopUtil() {}

  public static MethodInterceptor getMethodInterceptor(String targetClassUrl, String className) {
    Object obj = ClassUtil.getObj(targetClassUrl, className);
  ...
  public static <T> T getProxy(ProxyMetaInfo proxyMetaInfo) {
    ...
    MethodInterceptor methodInterceptor =
        AopUtil.getMethodInterceptor(
            proxyMetaInfo.getProxyUrl(), proxyMetaInfo.getProxyClassName());
    proxyFactory.addAdvisor(new DefaultPointcutAdvisor(pointcut, methodInterceptor));
    return proxyFactory.getProxy();
  }

  public static void registerProxy(
      DefaultListableBeanFactory beanFactory, ProxyMetaInfo proxyMetaInfo) {
    AspectJExpressionPointcutAdvisor advisor =
        getAspectJExpressionPointcutAdvisor(beanFactory, proxyMetaInfo);
  ...
  private static AspectJExpressionPointcutAdvisor getAspectJExpressionPointcutAdvisor(
    ...
    advisor.setAdvice(
        Objects.requireNonNull(
            getMethodInterceptor(proxyMetaInfo.getProxyUrl(), proxyMetaInfo.getProxyClassName())));
    ...
    for (String beanDefinitionName : beanFactory.getBeanDefinitionNames()) {
      ...
      if (operateEventEnum == OperateEventEnum.DEL) {
        if (isFindMatchAdvised) {
          ...
          log.info(
              "Remove Advice -->【{}】 For Bean -->【{}】 SUCCESS !",
  ...
  private static void buildCandidateAdvised(
    ...
    if (isFindMatchCandidateAdvised) {
      ...
      log.info(
          "BuildCandidateAdvised -->【{}】 With Advice -->{} SUCCESS !",
          bean.getClass().getName(),
          advisor.getAdvice().getClass().getName());
    }
  }
